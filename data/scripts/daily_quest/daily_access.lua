...
local player_positions = {
    [1] = {fromPos = Position(31869, 31986, 7), toPos = Position(31869, 32010, 8)},
    [2] = {fromPos = Position(31868, 31986, 7), toPos = Position(31869, 32011, 8)},
    [3] = {fromPos = Position(31867, 31986, 7), toPos = Position(31869, 32012, 8)},
    [4] = {fromPos = Position(31866, 31986, 7), toPos = Position(31869, 32013, 8)}
}
...
local specs, spec = Game.getSpectators(Position(33919, 31646, 8), false, false, 18, 18, 18, 18) -- Do not change
    for j = 1, #specs do
        spec = specs[j]
        if spec:getName():lower() == 'Bashmu' then
            spec:remove()
        end
    end
end
...
function earthArena.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    for i = 1, #participants do
        ...
        player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You have 30 minutes to kill and loot all monsters. Otherwise you will lose that chance and will be kicked out.")
    end
