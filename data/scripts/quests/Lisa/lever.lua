...
local player_positions = {
    [1] = {fromPos = Position(32030, 32664, 12), toPos = Position(32053, 32620, 12)},
    [2] = {fromPos = Position(32029, 32664, 12), toPos = Position(32054, 32621, 12)},
    [3] = {fromPos = Position(32028, 32664, 12), toPos = Position(32053, 32622, 12)},
    [4] = {fromPos = Position(32027, 32664, 12), toPos = Position(32054, 32622, 12)}
}

local monsters = {
    [1] = {pos = Position(32054, 32626, 12), name = "Lisa"}
}
...
local specs, spec = Game.getSpectators(Position(33919, 31646, 8), false, false, 18, 18, 18, 18)
    for j = 1, #specs do
        spec = specs[j]
        if spec:getName():lower() == 'Lisa' then
            spec:remove()
        end
    end
end
