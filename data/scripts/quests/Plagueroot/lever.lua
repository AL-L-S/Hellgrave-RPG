...
local player_positions = {
    [1] = {fromPos = Position(32122, 32668, 10), toPos = Position(32151, 32676, 10)},
    [2] = {fromPos = Position(32121, 32668, 10), toPos = Position(32152, 32676, 10)},
    [3] = {fromPos = Position(32120, 32668, 10), toPos = Position(32153, 32676, 10)},
    [4] = {fromPos = Position(32119, 32668, 10), toPos = Position(32154, 32676, 10)}
}

local monsters = {
    [1] = {pos = Position(32153, 32668, 10), name = "Plagueroot"}
}
...
local specs, spec = Game.getSpectators(Position(33919, 31646, 8), false, false, 18, 18, 18, 18)
    for j = 1, #specs do
        spec = specs[j]
        if spec:getName():lower() == 'Plagirath' then
            spec:remove()
        end
    end
end
